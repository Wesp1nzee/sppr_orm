...
Журналы аудита доступны только администратору. Сводный отчёт по результатам
проверки доступен владельцу проверки ИЛИ администратору — полным доступом
ограничены именно журналы, а не отчёт по собственной проверке пользователя.
"""
...
async def get_summary_report(
    ...
    используется ``CurrentUser`` (не ``require_roles(admin)``): полным доступом
    ограничены журналы аудита, а отчёт по собственной проверке — данные
    пользователя, поэтому он доступен и владельцу.
    """
